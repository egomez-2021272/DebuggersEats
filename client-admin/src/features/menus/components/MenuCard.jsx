...
export const MenuCard = ({ menu, isAdmin, onEdit, onDelete }) => (
    <article className="rounded-2xl overflow-hidden flex flex-col bg-[#16161f] border border-white/[0.07] hover:-translate-y-0.5 transition-all duration-300"
        style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.3' }}
    >
        <figure className="w-full h-40 flex items-center justify-center overflow-hidden bg-white/[0.04] m-0">
            {menu.photo ? <img src={menu.photo} alt={menu.name} className="w-full h-full object-cover" />
        ...
        <div className="p-4 flex flex-col flex-1 gap-2">
            <header className="flex items-start justify-between gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold shrink-0 ${CATEGORY_COLORS[menu.category] || 'bg-white/10 text-white'}`}>
            ...
            <p className="text-base font-bold text-white/90 mt-auto">
            ...
            {menu.ingredients?.length > 0 && (
                <ul className="flex flex-wrap gap-1" aria-label="Ingredientes">
                    {menu.ingredients.slice(0, 4).map((ing) => (
                        <li key={ing} className="px-2 py-0.5 rounded-full text-xs bg-white/[0.06] text-white/50">
                            {ing}
                        </li>
                    ))}
            ...
            {!menu.available && (
                <p className="text-xs text-red-400/70">No disponible</p>
            )}
            ...
            {isAdmin && (
                <footer className="flex gap-2 mt-2 pt-3 border-t border-white/[0.05]">
                    <button className="flex-1 py-1.5 rounded-lg text-sm font-medium transition bg-pink-500/15 hover:bg-pink-500/25 border border-pin-500/30 text-pink-400"
                        onClick={() => onEdit(menu)}>
                        Editar
                    </button>
                    <button
